...
public class TeacherRegistration extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   int i = st.executeUpdate("insert into teacher (email_id, pwd ,age , phone, gender ) values ('" + email_id + "','" 

+ pwd + "','" +age + "','" + phone + "','" + gender + "')");
	   
	   ResultSet rs;
	   
	   rs = st.executeQuery("select * from teacher  where email_id='" + email_id + "'");
	   ...
	   while(rs.next())
	   {
	    ...
	    if(i>0)
	    {
	    	...
	    	sendMail("javamailsystem1@gmail.com",email_id,"Account activation link",
	    			"Please click on the following link to activate your account  --> http://localhost:8080/EduNext/teacher_profile.jsp?key=",key);
	    
	    	directoryGenerator(email_id);
}
	   ...
	   catch(Exception e)
	   {
	    ...
	    public void directoryGenerator(String id)
	    {
	    	try{
		    	Class.forName("com.mysql.jdbc.Driver");
		    	  Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/EduNext","root","admin");
	    	...
	    	ResultSet rs2=st.executeQuery("select * from teacher where email_id='"+id+"'");
	   ...
	   if(flag==1)
	    	{	//C:\Users\karan Pc\workspace\EduNext
	    	File file = new File("C:\\Users\\karan Pc\\git\\EduNext\\WebContent\\FileUpload\\"+id+"");
	    	if (!file.exists())
	    	{
	    		if (file.mkdirs())
	    		{	
	    		...
	    		{
	    			System.out.println("Failed to create directory! or Folder already exist");
	    		}
	    	}
	    	}
	    	}
...
("mail.smtp.socketFactory.class"));
    ...
    */
    ...
    Session mySession = Session.getInstance(props, new Authenticator(){
    	...
    	{
    		return new PasswordAuthentication("javamailsystem1@gmail.com","teamissac12345");
